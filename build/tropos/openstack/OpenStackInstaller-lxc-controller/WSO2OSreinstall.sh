...
#
#    WSO2OSreinstall.sh - Simple bash script installer for Openstack
...
cat << USAGE
Syntax
    WSO2OSreinstall.sh -T {type} -s { network size } -n {number of networks} -p {public interface} -P {private interface} -f {floating_range} -F {fixed_range} -V {VLAN start} -C {Controller Address} -A {admin} -v {qemu | kvm} -t {default tenancy} -M {ip of MySQL service}
...
case ${INSTALL} in
	all|single)
		NOVA_PACKAGES="nova-api nova-objectstore nova-scheduler nova-network nova-compute-lxc nova-cert glance keystone openstack-dashboard memcached python-memcache novnc nova-consoleauth nova-console"
	...
	controller)
		...
		;;
	compute|node)
		NOVA_PACKAGES="nova-compute-lxc nova-network nova-api python-keystone"
		EXTRA_PACKAGES="euca2ools unzip qemu ntp python-dateutil"
		;;
esac
