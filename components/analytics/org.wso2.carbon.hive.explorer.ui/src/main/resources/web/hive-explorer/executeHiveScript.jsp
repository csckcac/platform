...
<%@ page import="org.wso2.carbon.analytics.hive.stub.HiveScriptStoreServiceStub" %>
...
<%
    ...
    if (auth == null) {
        if (driver != null && !driver.toString().equalsIgnoreCase("")) {
            try {
                String serverURL = CarbonUIUtil.getServerURL(config.getServletContext(), session);
                ConfigurationContext configContext =
                        (ConfigurationContext) config.getServletContext().getAttribute(CarbonConstants.CONFIGURATION_CONTEXT);
...
<%
        ...
        if (null != request.getParameter("scriptName")) {
...
<script type="text/javascript">
    location.href = '../hive-explorer/index.jsp?scriptName=' + '<%=scriptName%>';
</script>
<%
        }
    }
...
<div id="middle">
    <h2>Hive Explorer - <%=scriptName%></h2>
    ...
    <div id="workArea">
        ...
        <form id="commandForm" name="commandForm" action="" method="POST">
            <table class="styledLeft">
                ...
                <input type="hidden" name="scriptName" id="scriptName" value="<%=scriptName%>"/>
                </tbody>
            </table>
            </td>
            </tr>
