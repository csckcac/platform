...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:param name="tryit-service-stub-location" />
    ...
    <!-- This parameter was added in here to show a special SOAP action input boxes for ESB proxy services.
    In a proxy service case the SOAP action in the wsdl is really incorrect and for the tryit to work the
    user needs to specify the SOAP action-->
    <xsl:param name="show-soapAction" select="'false'"/>
    ...
    <xsl:template name="header-template">
        <xsl:call-template name="css"/>
            ...
            <xsl:variable name="src-tryit-service">
                <xsl:value-of select="$tryit-service-stub-location"/><xsl:if test="$e4x">&amp;lang=e4x</xsl:if>
            </xsl:variable>
...
<xsl:call-template name="browser-compatibility"/>
        ...
        function init() {
            ...
            <xsl:choose>
                <xsl:when test="boolean($endpoint)">
                      <xsl:value-of select="$service-name"/>.endpoint = '<xsl:value-of
                        select="$endpoint"/>';
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:value-of select="$service-name"/>.proxyAddress = '<xsl:value-of select="$proxyAddress"/>';
    ...
    <xsl:template name="body-template">
        <!-- insert breadcrumbs -->
            ...
            <div id="body">
                ...
                <xsl:if test="not($endpoint)">
                    <div id="endpoint">
                        <div id="endpoint-collapsed">
                            ...
                            <div id="proxyWarning1" style="display:none" class="warning"><strong>Note:</strong> Private proxy protocol will be attempted as cross-domain browser restrictions might be enforced for this endpoint.  <a
                                    href="http://wso2.org/library/articles/xss-it-restricted">More info...</a></div>
                            <div class="bottom">
                        ...
                        <div id="endpoint-expanded" style="display:none">
                            ...
                            <div id="proxyWarning2" style="display:none" class="warning"><strong>Note:</strong> Private proxy protocol will be attempted as cross-domain browser restrictions might be enforced for this endpoint.  <a
                                    href="http://wso2.org/library/articles/xss-it-restricted">More info...</a></div>
                            <div class="bottom">
                ...
                <xsl:if test="$show-soapAction='true'">
                    <div id="soapActionDiv">This service may require a custom SOAP action:
                        <div>SOAP Action:
                            <input type="text" id="soapAction" size="20"></input>
                        </div>
                    </div>
                </xsl:if>
                <div id="middle">
    ...
    <xsl:template name="do-operation-functions">
        ...
        <xsl:if test="$show-soapAction='true'">
            <xsl:value-of select="$service-name"/>._endpointDetails[<xsl:value-of select="$service-name"/>.endpoint].action[currentOperationName] = document.getElementById("soapAction").value;
            <xsl:value-of select="$service-name"/>._endpointDetails[<xsl:value-of select="$service-name"/>.endpoint].soapaction[currentOperationName] = document.getElementById("soapAction").value;
        </xsl:if>
    ...
    <xsl:template name="binding-config-functions">
                ...
                for (i=endpointName.options.length - 1; i>=0; i--) {
                    ...
                    if ((endpointScheme == 'http' || endpointScheme == 'https') &amp;&amp; endpointScheme != pageScheme) {
                        ...
                        if (!linkFixed) {
                            ...
                            link.href = pageUrl.replace(WebService.utils.scheme(pageUrl), endpointScheme).replace(WebService.utils.domainPort(pageUrl), endpointPort);
                            linkFixed = true;
                        }
                        var currentEndpointAddress = document.getElementById("address").value;
        ...
        function selectEndpoint() {
            ...
            <xsl:if test="$show-soapAction='true'">
            document.getElementById("soapAction").value = <xsl:value-of select="$service-name"/>._endpointDetails[endpoint].soapaction[currentOperationName];
            </xsl:if>
            addressChange();
            showRestTemplate();
        }
        ...
        function addressChange() {
            ...
            if (newEndpointAddress != null) {
                ...
                document.getElementById('proxyWarning2').style.display = xss ? "block" : "none";                
                showRestTemplate();
            }
        }
    </xsl:template>
