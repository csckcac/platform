...
/**
 * This JDBC based BAM activity processor can directly store generated
 * message activities in a database. This can be used as an alternative to
 * the usual eventing based activity discovery model.
 */
public class JDBCActivityProcessor implements ActivityProcessor {
    ...
    public JDBCActivityProcessor() {
        ...
        pm = new ActivityPersistenceManager();
    ...
    public void process(MessageActivity[] activities) {
        exec.submit(new JDBCWorker(activities));
    }
    ...
    private class JDBCWorker implements Runnable {
        ...
        public void run() {
            for (MessageActivity act : activities) {
                pm.persistActivity(act);
            }
        }
    }
}