...
public class ActivityEventQueue extends AbstractQueue {
    ...
    public ActivityEventQueue(int threadPoolSize) {
    ...
    protected void processEvents(MessageContext[] messageContexts) {
        ...
        for (int i = 0; i < messageContexts.length; i++) {
            SOAPBody body = messageContexts[i].getEnvelope().getBody();
            ...
            if (itr.hasNext()) {
                ...
                OMElement aidEventElement = messageContexts[i].getEnvelope().getBody().getFirstChildWithName(aidEventQname);
