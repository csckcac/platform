...
public class Grid {
	...
	public int getRowWidth(int rowIndex) {
		...
		for (ActivityElement<?> element : this.data.get(rowIndex)) {
			width += element.getDimension().getWidthWithMargin();
		}
	...
	public int getRowHeight(int rowIndex) {
		...
		for (ActivityElement<?> element : this.data.get(rowIndex)) {
			if (element.getDimension().getHeightWithMargin() > height) {
				height = element.getDimension().getHeightWithMargin();
			}
		}
	...
	private void addPlaceHolder(ActivityElement<?> newElement, int rowIndex) {
		...
		if (rowIndex1 != rowIndex2 && rowIndex1 >= 0 && rowIndex2 >= 0) {
			...
			if (placeHolderAmount > 0) {
				...
				ActivityElement<?> element = this.data.get(rowIndex1).get(columnCount - 1);
				this.data.get(rowIndex1).remove(element);

				addPlaceHolders(rowIndex1, placeHolderAmount);

				// Add the element to the list (after the place-holders
				this.data.get(rowIndex1).add(element);
			}
