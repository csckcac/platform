...
<table id="activity_omitter_by_name_table" class="adjuster-inner">
    <thead>
    <tr>
            ...
            <table class="inner light-orange">
                ...
                <tr>
                    <td class="inner left"><fmt:message
                            key="activity_name_omitting"/></td>
                    <td class="inner right">
                        <a name="svg_omitting_names_viewmode">
                            <%
                                if (bean.getSvgDataModel().getNameOmitting().getViewMode().isMaximised()) {
                            %>
    ...
    <tr>
        <td class="center">
            <select size="8" multiple="multiple"
                    name="activitynames_omitting_left">
                <%
                    for (SelectItem activityName : bean.getSvgDataModel().getNameOmitting().getItemsLeft()) {
                %>
        ...
        <td class="center">
            <input type="button" value="<fmt:message key="add" />"
                   name="activitynames_omitting_add" id="activitynames_omitting_add" class="activitynames_omitting">
            <br/>
            <input type="button" value="<fmt:message key="remove" />"
                   name="activitynames_omitting_remove" id="activitynames_omitting_remove" class="activitynames_omitting">
            <br/>
            <br/>
            <input type="button" value="<fmt:message key="addAll" />"
                   name="activitynames_omitting_addall" id="activitynames_omitting_addall" class="activitynames_omitting">
            <br/>
            <input type="button" value="<fmt:message key="removeAll" />"
                   name="activitynames_omitting_removeall" id="activitynames_omitting_removeall" class="activitynames_omitting">
        </td>
        <td class="center">
            <select size="8" multiple="multiple"
                    name="activitynames_omitting_right">
                <%
                    for (SelectItem activityName : bean.getSvgDataModel().getNameOmitting().getItemsRight()) {
                %>
                <option value="<%=activityName.getValue()%>"><%=activityName.getValue()%>
                </option>
                <%
...
<script type="text/javascript">
    $("input[class=activitynames_omitting]").click(function() {
        submitSettings(this.name);
        reloadTable("activity_omitter_by_name_table");
        reloadSVG();
    });

     $("a[name=svg_omitting_names_viewmode]").click(function() {
        minMax(this.name);
        reloadTable("activity_omitter_by_name_table");
    });
</script>
