...
(function() {
	...
	function fdSlider(inp, range, callback, classname, hide, tween, vertical) {
		...
		this.events = {
			stopevent : function(e) {
				...
				/* @cc_on@ */
				/*
				 * @if(@_win32) e.cancelBubble = true; e.returnValue = false;
				 * /*@end@
				 */
				return false;
			},
			redrawEvent : function(e) {
				// Get around IE's window.resize bug by testing if the actual
				// size of the wrapper has changed...
				if ((self._vertical && self.outerWrapper.offsetHeight != self._resizeVal)
						|| (!self._vertical && self.outerWrapper.offsetWidth != self._resizeVal)) {
					self.events.redraw();
				}
				;
			},
			...
			onblur : function(e) {
				self.outerWrapper.className = self.outerWrapper.className.replace(
						/focused|fd-fc-slider-hover|fd-slider-hover/g, '');
			...
			trackmousewheel : function(e) {
				...
				if (delta) {
					...
					var wtmp = self._vertical ? self.outerWrapper.offsetHeight - self.handle.offsetHeight
							: self.outerWrapper.offsetWidth - self.handle.offsetWidth;
					var inc = self._inp.tagName.toLowerCase() == "input" ? Math.round(self._incPx / 2) < 1 ? 1
							: Math.round(self._incPx / 2)
							: self._incPx;
					if (self._vertical)
						inc = -inc;
					if (delta < 0) {
			...
			onkeydown : function(e) {
				...
				var wtmp = self._vertical ? self.outerWrapper.offsetHeight - self.handle.offsetHeight
						: self.outerWrapper.offsetWidth - self.handle.offsetWidth;
				var inc = self._inp.tagName.toLowerCase() == "input" ? Math.round(self._incPx / 2) < 1 ? 1
						: Math.round(self._incPx / 2) : self._incPx;
			...
			onmouseover : function(e) {
				 ...
				 * if(this.className.search(/focused/) != -1) { this.className =
				 * this.className.replace("fd-fc-slider-hover", "") +'
				 * fd-fc-slider-hover'; return; } /*@end@
				 */
			...
			onmouseout : function(e) {
				 ...
				 * @if(@_jscript_version <= 5.6)
				 * if(this.className.search(/focused/) != -1) { this.className =
				 * this.className.replace("fd-fc-slider-hover", ""); return; }
				 * /*@end@
				 */
			...
			onHmouseup : function(e) {
				...
				// Opera fires the blur event when the mouseup event occurs on a
				// button, so we attept to force a focus
				if (window.opera)
					try {
						setTimeout(function() {
							self.events.onfocus();
			...
			onHmousedown : function(e) {
				...
				// Safari will not "focus" on the button on mouse events, so we
				// attempt to force a focus
			...
			trackmouse : function(e) {
				if (!e)
					var e = window.event;
				var x = self._vertical ? self.handleX + (e.clientY - self._mouseX) : self.handleX
						+ (e.clientX - self._mouseX);
				if (x < 0)
					x = 0;
				var max = self._vertical ? self.outerWrapper.offsetHeight - self.handle.offsetHeight
						: self.outerWrapper.offsetWidth - self.handle.offsetWidth;
				if (x > max)
					x = max;
				if (self._vertical)
			...
			var diff = Math.round((self._vertical) ? self.handle.offsetHeight / 2
					: self.handle.offsetWidth / 2);
		...
		this.resetHandlePosition = function() {
			var value = self._inp.tagName.toLowerCase() == "input" ? parseInt(self._inp.value, 10)
					: self._inp.selectedIndex;
			...
			var pos = (tot * inc < (self._min + (self._range / 2))) ? Math.floor(tot * inc) : Math.ceil(tot
					* inc);
			if (self._vertical) {
		...
		this.findLabel = function() {
			...
			if (self._inp.parentNode && self._inp.parentNode.tagName.toLowerCase() == "label")
				...
				for ( var lbl = 0; lbl < labelList.length; lbl++) {
					...
					if ((labelList[lbl]['htmlFor'] && labelList[lbl]['htmlFor'] == self._inp.id)
							|| (labelList[lbl].getAttribute('for') == self._inp.id)) {
						label = labelList[lbl];
						break;
					}
					;
		...
		this.build = function() {
			...
			self.outerWrapper.className = "fd-slider" + (self._vertical ? "-vertical " : " ")
					+ self._classname;
			...
			/* @cc_on@ */
			/*
			 * @if(@_win32) self.handle.unselectable = "on";
			 * self.bar.unselectable = "on"; self.wrapper.unselectable = "on";
			 * self.outerWrapper.unselectable = "on"; /*@end@
			 */
			...
			// self.setAttrNS(self.handle, self.NS_STATE, "valuenow",
			// self._inp.value); // aaa:valuenow
			...
			if (lbl) {
				...
				/*
				 * @cc_on /*@if(@_win32) lbl.setAttribute("htmlFor",
				 * self.handle.id); @else @
				 */
			...
			// Are there page instructions - the creation of the instructions
			// has been left up to you fine reader...
	...
	fdSliderController = {
		...
		construct : function(e) {
			...
			var inputs = fdSliderController.joinNodeLists(document.getElementsByTagName('input'), document
					.getElementsByTagName('select'));
			var range, callback, classname, hide, tween, vertical;

			for ( var i = 0, inp; inp = inputs[i]; i++) {
				if ((inp.tagName.toLowerCase() == "input" && inp.type == "text" && inp.className && inp.className
						.search(regExp_1) != -1)
						|| (inp.tagName.toLowerCase() == "select" && inp.className.search(/fd_slider/) != -1)) {
					...
					fdSliderController.sliders[inp.id] = new fdSlider(inp, range, callback, classname, hide,
							tween, vertical);
				}
				;
			}
		...
		deconstruct : function(e) {
			...
			/* @cc_on@ */
			/*
			 * @if(@_win32) fdSliderController.removeEvent(window, "load",
			 * function() { setTimeout(fdSliderController.onload, 200) });
			 * /*@end@
			 */
		}
	}
...
/* @cc_on@ */
/*
 * @if(@_win32) fdSliderController.addEvent(window, "load", function() {
 * setTimeout(fdSliderController.onload, 200) }); /*@end@
 */
