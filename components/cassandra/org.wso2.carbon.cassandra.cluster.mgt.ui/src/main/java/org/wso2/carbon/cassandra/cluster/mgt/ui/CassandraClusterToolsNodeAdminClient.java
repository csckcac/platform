...
/**
 * A client to access CassandraClusterToolsAdmin service
 */
public class CassandraClusterToolsNodeAdminClient {

    private static final Log log = LogFactory.getLog(CassandraClusterToolsNodeAdminClient.class);

    private CassandraClusterAdminStub cassandraAdminStub;



    public CassandraClusterToolsNodeAdminClient(ConfigurationContext ctx, String serverURL, String cookie)
            throws AxisFault {
        init(ctx, serverURL, cookie);
    }

    public CassandraClusterToolsNodeAdminClient(javax.servlet.ServletContext servletContext,
     ...
     * @throws CassandraClusterToolsAdminClientException for errors during getting node informations
     */
    public NodeInformation[] listNodes() throws CassandraClusterToolsAdminClientException {
        try {
            return cassandraAdminStub.getNodes();
        } catch (Exception e) {
            throw new CassandraClusterToolsAdminClientException("Error retrieving nodes names !", e, log);
        }
    }
    ...
    private void validateCF(String columnFamilyName) throws CassandraClusterToolsAdminClientException {
        if (columnFamilyName == null || "".equals(columnFamilyName.trim())) {
            throw new CassandraClusterToolsAdminClientException("The column family name name is empty or null", log);
        }
