 ...
 */
public class CassandraClusterClusterToolsDataAccessMBeanImplementation implements CassandraClusterToolsMBeanDataAccess {

    private static final Log log = LogFactory.getLog(CassandraClusterClusterToolsDataAccessMBeanImplementation.class);
    ...
    //private CassandraClusterToolsMBeanServerConnection cassandraClusterToolsMBeanServerConnection;

    public CassandraClusterClusterToolsDataAccessMBeanImplementation() {
        this.mBeanServerConnection = new CassandraClusterToolsMBeanServerConnection().getMBeanServerConnection();
    }
     ...
     * @throws CassandraClusterToolsDataAccessException
     *          for error during locating <code>StorageServiceMBean </code>
     */
    public StorageServiceMBean locateStorageServiceMBean() throws
                                                           CassandraClusterToolsDataAccessException {

        if (storageServiceProxy == null) {
            storageServiceProxy = locateMBean(CassandraClusterToolsDataAccessConstants.STORAGE_SERVICE_OBJECT_NAME, StorageServiceMBean.class);
        }
     ...
     */
    public StreamingServiceMBean locateStreamingServiceMBean() throws
                                                               CassandraClusterToolsDataAccessException {
        if (streamProxy == null) {
     ...
     */
    public CompactionManagerMBean locateCompactionManagerMBean() throws
                                                                 CassandraClusterToolsDataAccessException {
     ...
     */
    public ColumnFamilyStoreMBean locateColumnFamilyStoreMBean(String ks, String cf) throws
                                                                                     CassandraClusterToolsDataAccessException {
    ...
    /*
    @Override
    public void createRemoteJmxConnection(String username, String password, int jmxPort, String host) throws CassandraClusterToolsDataAccessException {
        cassandraClusterToolsMBeanServerConnection = new CassandraClusterToolsMBeanServerConnection();
     ...
     */
    private <T> T locateMBean(String name, Class<T> mBeanClass) throws
                                                                CassandraClusterToolsDataAccessException {
        try {
            return JMX.newMBeanProxy(mBeanServerConnection,
                                     new ObjectName(name), mBeanClass);
        } catch (MalformedObjectNameException e) {
            throw new CassandraClusterToolsDataAccessException(
