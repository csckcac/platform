...
public abstract class BaseTestCase extends TestCase {
	...
	protected void setUp() throws Exception {
		if (!started) {
			File tmpFile = File.createTempFile("zoo", "keeper");
			tmpFile.delete();
			tmpFile.mkdir();
			tmpFile.deleteOnExit();
			String config = "tickTime=10000\ndataDir=" + tmpFile.getAbsolutePath()
					+ "\nclientPort=2181";
			CoordinationServer server = new CoordinationServer(config);
			server.start();
