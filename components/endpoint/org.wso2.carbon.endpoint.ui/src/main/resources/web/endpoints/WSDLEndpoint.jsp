...
<%@ page import="org.wso2.carbon.endpoint.common.to.*" %>
...
<%@ page import="org.apache.synapse.endpoints.WSDLEndpoint" %>
...
<%@ page import="org.apache.synapse.config.xml.endpoints.WSDLEndpointFactory" %>
...
<%
    WSDLEndpointData wsdlEpData = null;
...
//    System.out.println("tempalteADD: " + templateAdd);
    ...
    if (endpointAction != null && !"".equals(endpointAction) && endpointAction.equals("edit")) {
        session.setAttribute("edit" + endpointName, "edit");
        try {
            if (isFromTemplateEditor) {
                wsdlEpData = adminClient.getWSDLEndpoint(templateObj, fac);
                session.setAttribute("edit" + templateName, "edit"); // uses when saving the template
            }
            else{
                wsdlEpData = adminClient.getWSDLEndpoint(endpointName);
            }

        } catch (Exception e) {
            String msg = "Unable to get WSDL endpoint data: " + e.getMessage();
        ...
        try {
            ...
            if (isFromTemplateEditor) {
                ...
                WSDLEndpoint wsdlEp = (WSDLEndpoint) EndpointFactory.getEndpointFromElement(templateObj.getElement(), fac,false, properties);
                wsdlEpData = EndpointConfigurationHelper.getWSDLEpFromSynEp(wsdlEp, session, isAnonymous);
            }
            else{
                WSDLEndpoint wsdlEp = (WSDLEndpoint) EndpointFactory.getEndpointFromElement(element, false, new Properties());
                wsdlEpData = EndpointConfigurationHelper.getWSDLEpFromSynEp(wsdlEp, session, isAnonymous);
            }
        ...
        if (anonymousEndpointXML != null && !"".equals(anonymousEndpointXML)) {
            ...
            try {
                wsdlEpData = EndpointAdminClient.
                        getWsdlEndpointData(anonymousEndpointXML);
            } catch (Exception e) {
...
<%
    ...
    if (wsdlEpData != null) {
        ...
        // Endpoint Name
        if (wsdlEpData.getEpName() != null) {
            wsdlEpName = wsdlEpData.getEpName();
        }

         // Description
        if (wsdlEpData.getDescription() != null) {
            description = wsdlEpData.getDescription();
        }

        // Inline WSDL ?
        if (wsdlEpData.getInLineWSDL() != null &&
                !"".equals(wsdlEpData.getInLineWSDL())) {
            isInlineWSDL = true;
            inlineWSDl = wsdlEpData.getInLineWSDL();
        } else {
            isInlineWSDL = false;
            if (wsdlEpData.getEpUri() != null) {
                wsdlURI = wsdlEpData.getEpUri();
            }
        }

        // Service
        if (wsdlEpData.getEpServ() != null) {
            service = wsdlEpData.getEpServ();
        }

        // Port
        if (wsdlEpData.getEpPort() != null) {
            port = wsdlEpData.getEpPort();
        }

        // Error codes
        if (wsdlEpData.getEperrorCodes() != null) {
            errorCode = wsdlEpData.getEperrorCodes().trim();
        }

        // Initial duration
        if (wsdlEpData.getEpDur() >= 0) {
            suspendDurationOnFailure = wsdlEpData.getEpDur();
        }

        // Max duration
        if (0 <= wsdlEpData.getEpmaxSusDuration() && wsdlEpData.getEpmaxSusDuration() < Long.MAX_VALUE) {
            maxDuration = wsdlEpData.getEpmaxSusDuration();
        }

        // Factor
        if (0 <= wsdlEpData.getEpsusProgFactor() && wsdlEpData.getEpsusProgFactor() >= 0.0) {
            factor = wsdlEpData.getEpsusProgFactor();
        }

        // TimeOut error code
        if (wsdlEpData.getEptimdedOutErrorCodes() != null) {
            timeOutErrorCode = wsdlEpData.getEptimdedOutErrorCodes().trim();
        }

        //non-retry error codes
         if(wsdlEpData.getRetryDisabledErrorCodes() != null) {
             retryDisabledErrorCodes = wsdlEpData.getRetryDisabledErrorCodes().trim();
         }

        // Retry time out
        if (wsdlEpData.getEpretryTimeout() > 0) {
            retryTimeOut = wsdlEpData.getEpretryTimeout();
        }

        // Retry delay timeout
        if (wsdlEpData.getEpretryDelay() > 0) {
            retryDelayTimeOut = wsdlEpData.getEpretryDelay();
        }

        // Action
        if (wsdlEpData.getEpwsdlTimeoutAction() == 100) { // TODO-remove magic number
            isNeverTimeout = true;
        } else if (wsdlEpData.getEpwsdlTimeoutAction() == 101) { // TODO-remove magic number
            isDiscardMessage = true;
            actionDuration = wsdlEpData.getEpactionDuration();
        } else {
        ...
        if (wsdlEpData.isEpaddressingOn()) {
            isEnableAddressing = true;
        }

        // Use seperate listener ?
        if (wsdlEpData.isEpwsaddSepListener()) {
            isSepListener = true;
        }

        // Enable WS-Sec?
        if (wsdlEpData.isEpsecutiryOn()) {
            isEnableWSSec = true;
            // Sec. policy
            if (wsdlEpData.getEpwsdlSecutiryKey() != null) {
                secPolicy = wsdlEpData.getEpwsdlSecutiryKey();
            }
        }

        // Enable rm ?
        if (wsdlEpData.isEprelMesg()) {
            isEnableRM = true;
            // RM. policy
            if (wsdlEpData.getEprmKey() != null) {
                rmPolicy = wsdlEpData.getEprmKey();
            }
        }

        if (wsdlEpData.getProperties() != null && wsdlEpData.getProperties() != "") {
            properties = wsdlEpData.getProperties();
        }
    }
%>
...
<script type="text/javascript">
    ...
    // invoke when saving WSDL data
    function submitEndpointData(isAnonymous) {
        if (!isValidAddressORWSDLEndpoint(isAnonymous, 'wsdl')) {
            <%
        ...
        var propstr = populateServiceParams("headerTable");
	if (!isValidProperties(propstr)) {
	   CARBON.showWarningDialog('Missing property name or value');
	   return false;
	}
	document.wsdlEndpointForm.endpointProperties.value = propstr;
        document.wsdlEndpointForm.action = 'saveEndpoint.jsp?endpointType=WSDLEndpoint';
        document.wsdlEndpointForm.submit();
        return true;
    }

    function submitDynamicEndpointData() {
        if (!isValidAddressORWSDLEndpoint(true, 'wsdl')) {
            <%
                 //ignore any ui validation for templates ie:- due to parameterized arguments present $uri,etc
                 //may show error messages
                 if (!isFromTemplateEditor) {
        ...
        document.wsdlEndpointForm.endpointProperties.value = populateServiceParams("headerTable");
        document.wsdlEndpointForm.action = 'saveDynamicEndpoint.jsp?registry='+registry
                +'&endpointType=WSDLEndpoint&regKey=' + key;
        document.wsdlEndpointForm.submit();
        return true;
    }
	...
	document.wsdlEndpointForm.endpointProperties.value = propstr;
        document.wsdlEndpointForm.action = 'endpointSourceView.jsp?endpointType=WSDLEndpoint&ordinal=1';
        document.wsdlEndpointForm.submit();
        return true;
    }
    ...
    function testWSDLURL(url) {
        if (url == '') {
            CARBON.showWarningDialog('<fmt:message key="invalid.address.empty"/>');
        } else {
            jQuery.get("testConnection-ajaxprocessor.jsp?type=wsdl&", {'url' : url},
                    function(data, status) {
                        if (data.replace(/^\s+|\s+$/g, '') == 'success') {
                            CARBON.showInfoDialog('<fmt:message key="valid.address"/> ' + url);
                        } else if (data.replace(/^\s+|\s+$/g, '') == 'unknown') {
                            CARBON.showErrorDialog('<fmt:message key="unknown.address"/> ' + url);
                        } else if (data.replace(/^\s+|\s+$/g, '') == 'malformed') {
                            CARBON.showErrorDialog('<fmt:message key="malformed.address"/> ' + url);
                        } else if (data.replace(/^\s+|\s+$/g, '') == 'ssl_error') {
                            CARBON.showErrorDialog('<fmt:message key="ssl.error"/> ' + url);
                        } else {
                            CARBON.showErrorDialog('<fmt:message key="invalid.address"/>');
                        }
                    });
        }
    }
    ...
    <h2>
        <% if (request.getParameter("serviceName") != null) {
           ...
           if ("edit".equals(endpointAction) && isFromTemplateEditor) {
            ...
            %><fmt:message key="wsdl.endpoint.template"/><%
           } else {
            %><fmt:message key="wsdl.endpoint"/><%
           }
        %>
    </h2>

<div id="workArea">
<form id="wsdlEnpointFormId" name="wsdlEndpointForm" action="" method="POST">
<table class="styledLeft">
<thead>
<tr>
    <th>
        <span style="float: left; position: relative; margin-top: 2px;">
                            <fmt:message key="design.view.of.the.wsdl.endpoint"/>
        </span>
        <a class="icon-link" style="background-image: url(images/source-view.gif);"
           onclick="switchToSource('<%=isAnonymous%>')" href="#"><fmt:message key="switch.to.source.view"/></a>
    </th>
</tr>
</thead>
<tbody>
...
<table class="normal-nopadding">
    ...
    <tr style="<%=!isAnonymous?"":"display:none"%>">
        ...
        <td><input id="endpointName" name="endpointName"
                   value="<%=wsdlEpName%>"
                <%=(!"".equals(wsdlEpName)) ? "disabled=\"disabled\"" : ""%>
                   <%=isFromTemplateEditor?"":"onkeypress=\"return validateText(event);\""%>   size="50"/>
            <input type="hidden" name="endpointName" value="<%=wsdlEpName%>"/>
        </td>
    </tr>
    ...
    <tr id="wsdlURIID" style="<%=!isInlineWSDL?"":"display:none;"%>">
        ...
        <td>
            ...
            <input id="testAddress" name="testAddress" type="button" class="button"
                   onclick="testWSDLURL(document.getElementById('uriWSDLVal').value)" value="Test URI"/>
        </td>
    </tr>
...
<tr>
    ...
    <td><input onclick="showHideOnSelect('wsRM','tr_ws_rm_policy_key')" type="checkbox" id="wsRM" name="wsRM" value="wsRM"
...
<div style="display:none;" id="saveAsForm">
    ...
    <table class="styledLeft">
        ...
        <tr>
            <td class="buttonRow">
                <input type="button" class="button" value="<fmt:message key="save"/>" id="saveSynRegButton"
                       onclick="javascript:submitDynamicEndpointData(); return false;"/>
                <input type="button" class="button" value="<fmt:message key="cancel"/>" id="cancelSynRegButton"
