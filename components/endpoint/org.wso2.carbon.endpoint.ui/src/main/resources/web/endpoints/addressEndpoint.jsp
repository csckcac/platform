...
<carbon:breadcrumb
        ...
        request="<%=request%>"/>
<%
    AddressEndpointData addressEpData = null;
...
//    System.out.println("tempalteADD: " + templateAdd);
    ...
    if (endpointAction != null && !"".equals(endpointAction) && endpointAction.equals("edit")) {
        ...
        try {
            if (isFromTemplateEditor) {
                addressEpData = adminClient.getAddressEndpoint(templateObj, fac);
                session.setAttribute("edit" + templateName, "edit"); // uses when saving the template
            }
            else{
                addressEpData = adminClient.getAddressEndpoint(endpointName);
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (isFromTemplateEditor) {
                ...
                AddressEndpoint addressEp = (AddressEndpoint) EndpointFactory.getEndpointFromElement(templateObj.getElement(), fac, false, new Properties());
                addressEpData = EndpointConfigurationHelper.getAddressEpFromSynEp(addressEp, session, isAnonymous);
            }
            else{
                AddressEndpoint addressEp = (AddressEndpoint) EndpointFactory.getEndpointFromElement(element, false, new Properties());
                addressEpData = EndpointConfigurationHelper.getAddressEpFromSynEp(addressEp, session, isAnonymous);
            }
        ...
        if (anonymousEndpointXML != null && !"".equals(anonymousEndpointXML)) {
            ...
            try {
                addressEpData = EndpointAdminClient.
                        getAddressEndpointData(anonymousEndpointXML);
            } catch (XMLStreamException e) {
...
<%
    ...
    if (addressEpData != null) {
        // Endpoint Name
        if (addressEpData.getEpName() != null) {
            addressEpName = addressEpData.getEpName();
        }

        // Endpoint Address
        if (addressEpData.getAddress() != null) {
            endpointAddress = addressEpData.getAddress();
            validAddressURL = EndpointConfigurationHelper.getValidXMLString(endpointAddress);
        }

        // Description
        if (addressEpData.getDescription() != null) {
            description = addressEpData.getDescription();
        }


        // Format string
        if (addressEpData.isPox()) {
            isPox = true;
        } else if (addressEpData.isRest()) {
            isRest = true;
        } else if (addressEpData.isSoap11()) {
            isSoap11 = true;
        } else if (addressEpData.isSoap12()) {
            isSoap12 = true;
        } else if (addressEpData.isGet()) {
            isGet = true;
        } else {
        ...
        // Optimize string
        if (addressEpData.isMtom()) {
            isMTOM = true;
        } else if (addressEpData.isSwa()) {
            isSWA = true;
        } else {
        ...
        // Error codes
        if (addressEpData.getErrorCodes() != null) {
            errorCode = addressEpData.getErrorCodes().trim();
        }

        // Initial duration
        if (addressEpData.getSuspendDurationOnFailure() >= 0) {
            suspendDurationOnFailure = addressEpData.getSuspendDurationOnFailure();
        }

        // Max duration
        if (0 <= addressEpData.getMaxSusDuration() && addressEpData.getMaxSusDuration() < Long.MAX_VALUE) {
            maxDuration = addressEpData.getMaxSusDuration();
        }

        // Factor
        if (0 <= addressEpData.getSusProgFactor() && addressEpData.getSusProgFactor() >= 0.0) {
            factor = addressEpData.getSusProgFactor();
        }

        // TimeOut error code
        if (addressEpData.getTimdedOutErrorCodes() != null) {
            timeOutErrorCode = addressEpData.getTimdedOutErrorCodes().trim();
        }

        //non-retry error codes
         if(addressEpData.getRetryDisabledErrorCodes() != null) {
             retryDisabledErrorCodes = addressEpData.getRetryDisabledErrorCodes().trim();
         }

        // Retry time out
        //this is the delay between retries for a timeout-out on a endpoint
        if (addressEpData.getRetryTimeout() > 0) {
            retryTimeOut = addressEpData.getRetryTimeout();
        }

        // Retry delay timeout
        //this is no of reties before suspend
        if (addressEpData.getRetryDelay() > 0) {
            retryDelayTimeOut = addressEpData.getRetryDelay();
        }

        // Action
        if (addressEpData.getTimeoutAct() == 100) { // TODO-remove magic number
            isNeverTimeout = true;
        } else if (addressEpData.getTimeoutAct() == 101) { // TODO-remove magic number
            isDiscardMessage = true;
            actionDuration = addressEpData.getTimeoutActionDur();
        } else {
        ...
        if (addressEpData.isWsadd()) {
            isEnableAddressing = true;
        }

        // Use seperate listener ?
        if (addressEpData.isSepList()) {
            isSepListener = true;
        }

        // Enable WS-Sec?
        if (addressEpData.isWssec()) {
            isEnableWSSec = true;
            // Sec. policy
            if (addressEpData.getSecPolKey() != null) {
                secPolicy = addressEpData.getSecPolKey();
            }
        }

        // Enable rm ?
        if (addressEpData.isWsrm()) {
            isEnableRM = true;
            // RM. policy
            if (addressEpData.getRmPolKey() != null) {
                rmPolicy = addressEpData.getRmPolKey();
            }
        }

        if (addressEpData.getProperties() != null && addressEpData.getProperties() != "") {
            properties = addressEpData.getProperties();
        }
    }
%>
...
<script type="text/javascript">
	...
	document.addressEndpointForm.endpointProperties.value = propstr
        document.addressEndpointForm.action = 'saveEndpoint.jsp?endpointType=addressEndpoint';
        document.addressEndpointForm.submit();
        return true;
    }
	...
	document.addressEndpointForm.endpointProperties.value = propstr;
        document.addressEndpointForm.action = 'saveDynamicEndpoint.jsp?registry='+registry
                +'&endpointType=addressEndpoint&regKey=' + key;
        document.addressEndpointForm.submit();
        return true;
    }
    ...
    function switchToSource(isAnonymous) {
        ...
        document.addressEndpointForm.endpointProperties.value = populateServiceParams("headerTable");
        document.addressEndpointForm.action = 'endpointSourceView.jsp?endpointType=addressEndpoint&retainlastbc=true';
        document.addressEndpointForm.submit();
        return true;
    }
...
<div id="workArea">
<form id="addressEndpointForm" name="addressEndpointForm" action="" method="POST">
        ...
        <td colspan="2">
            <a href="#" onclick="addServiceParams('headerTable')"
               style="background-image: url('../admin/images/add.gif');" class="icon-link">Add
                Property</a><input type="hidden" name="endpointProperties" id="endpointProperties"/>
        </td>
    </tr>
    <tr>
