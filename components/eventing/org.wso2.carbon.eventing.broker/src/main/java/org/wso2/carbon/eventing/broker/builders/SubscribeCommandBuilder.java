...
import org.wso2.eventing.EventCommandBuilder;
...
public class SubscribeCommandBuilder extends CommandBuilderConstants
        implements EventCommandBuilder<SOAPEnvelope> {
    ...
    public SOAPEnvelope fromSubscription(Subscription subscription) {
        ...
        subscriptionManagerEPR.addReferenceParameter(new QName(WSE_EVENTING_NS,
                WSE_EN_IDENTIFIER, WSE_EVENTING_PREFIX),
                subscription.getId());
        OMNamespace eventingNamespace = factory.createOMNamespace(WSE_EVENTING_NS,
                WSE_EVENTING_PREFIX);
        OMElement subscribeResponseElement = factory.createOMElement(
                WSE_EN_SUBSCRIBE_RESPONSE, eventingNamespace);
        try {
            OMElement subscriptionManagerElement = EndpointReferenceHelper.toOM(
                    ...
                    new QName(WSE_EVENTING_NS,
                            WSE_EN_SUBSCRIPTION_MANAGER,
                            WSE_EVENTING_PREFIX),
                    AddressingConstants.Submission.WSA_NAMESPACE);
            subscribeResponseElement.addChild(subscriptionManagerElement);
            OMElement expiresElement =
                    factory.createOMElement(WSE_EN_EXPIRES, eventingNamespace);
