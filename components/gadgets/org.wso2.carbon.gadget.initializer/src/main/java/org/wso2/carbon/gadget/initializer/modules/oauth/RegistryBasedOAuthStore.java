        ...
        try {
            ...
            if (arr.length > 0) {
                for (int i = 0; i < arr.length; i++) {
                    res = reg.get(arr[i]);
                    realStoreConsumerInfo(null, res.getProperty(OAuthConstants.CONSUMER_SERVICE), res);
                }
            }
        } catch (Exception e) {
...
/*    private void storeConsumerInfos(URI gadgetUri, JSONObject oauthConfig)
    ...
    private void realStoreConsumerInfo(URI gadgetUri, String serviceName, Resource consumerInfo)
            throws Exception {
        String callbackUrl = consumerInfo.getProperty(OAuthConstants.CALLBACK_URL);
        String consumerSecret = consumerInfo.getProperty(OAuthConstants.CONSUMER_SECRET_KEY);
        String consumerKey = consumerInfo.getProperty(OAuthConstants.CONSUMER_KEY_KEY);
        String keyTypeStr = consumerInfo.getProperty(OAuthConstants.KEY_TYPE_KEY);
...
//        index.setGadgetUri(gadgetUri.toASCIIString());
    ...
    public void setConsumerKeyAndSecret(
            GSOAuthStoreConsumerIndex providerKey,
