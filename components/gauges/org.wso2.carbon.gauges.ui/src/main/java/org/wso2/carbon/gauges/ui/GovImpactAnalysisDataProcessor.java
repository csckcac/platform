...
import org.wso2.carbon.gauges.ui.governance.gadgets.impactanalysis.beans.xsd.*;
...
//		if (bean.getServiceBean() == null) return "";
    ...
    private Map<String, Map<String, ServiceBean>> buildServiceVersionHierarchy(
        ...
        for (ServiceBean serviceBean : serviceBeans) {
            if (serviceBean != null) {
                String path = serviceBean.getPath();
                String version = "";
                for (int j = 0; j < 3; j++) {
                    path = RegistryUtils.getParentPath(path);
                    version = "." + RegistryUtils.getResourceName(path) + version;
                }
                ...
                versionMap.put(version.substring(1), serviceBean);
            }
        }
        return hierarchy;
    }
    ...
    private String buildImpactJSONTree(ServiceBean[] serviceBeans) {
        ...
        for (Map.Entry<String, Map<String, ServiceBean>> e : serviceVersionHierarchy.entrySet()) {
            ...
            for (String key : versionMap.keySet()) {
                countInner++;
                ServiceBean serviceBean = versionMap.get(key);
                if (countInner == 1) {
                ...
                builder.append("{" + buildOpenJSONWithEdges(uuid, key, serviceBean.getId(), "", getResourceURL(serviceBean.getPath() + "/" + key.replace(".", "/") + "/service")) + appendChildrenJSON() + "\n");
                        ...
                        for (int j = 0; j < wsdlBeans.length; j++) {
                            if (wsdlBeans[j] != null) {
                                ...
                                if (attachedSchemas != null) {
                                    ...
                                    for (int k = 0; j < attachedSchemas.length; j++) {
                                        if (attachedSchemas[j] != null) {
                                            builder.append("{"
                                                    + buildJSONwithNoChildren(attachedSchemas[j]
                                                    .getId(),
                                                    attachedSchemas[j].getQName(),
                                                    attachedSchemas[j].getPath()) + "} ");
                                            if (j != (attachedSchemas.length - 1))
                                                builder.append(",");
                                            builder.append("\n");
                                        }
    ...
    private class jsonTreeNode {
        private String id;
        private String name;
        public jsonTreeNode(PolicyBean policyBean) {
            this.id  = policyBean.getId();
            this.name = policyBean.getQName();
        }
