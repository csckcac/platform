        ...
        <form id="life.cycle.source.form" method="post" action="save_lcm-ajaxprocessor.jsp">
            <table class="styledLeft" cellspacing="0" cellpadding="0">
                ...
                <tr>
                    <td class="buttonRow">
                        <% if (!viewMode) { %>
                        <input class="button registryWriteOperation" type="button" onclick="saveAndValidateLC('<%=request.getParameter("lifecycleName")%>', <%=Boolean.toString(isNew)%>,'false')" value="<fmt:message key="save"/>"/>
