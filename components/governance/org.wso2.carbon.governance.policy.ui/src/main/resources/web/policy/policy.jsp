...
<script type="text/javascript" src="../policy/js/policy_util.js"></script>
<fmt:bundle basename="org.wso2.carbon.governance.policy.ui.i18n.Resources">
<%
...
%>
        <script type="text/javascript">
            CARBON.showErrorDialog("<fmt:message key="unable.to.upload.policy"/> " + "<%=(error != null) ? error : ""%>");
        </script>
<%
    }
%>
<script type="text/javascript">
    ...
    function handleFailure(o) {
        ...
        if (o.responseText) {
            CARBON.showErrorDialog("<fmt:message key="unable.to.upload.policy"/> "+o.responseText);
        } else {
            CARBON.showErrorDialog("<fmt:message key="unable.to.upload.policy"/>");
        }
    }

    function submitImportFormAsync() {

        var form = document.getElementById("policyImportForm");
    ...
    function submitUploadForm() {
        var form = document.getElementById("policyUploadForm");
        form.submit();
    }
    ...
    function addPolicy() {
        ...
        if (selectedValue == "upload") {
            ...
            reason += validateEmpty(irFetchURL, "<fmt:message key="policy.url"/>");
            if (reason == "") {
                reason += validateUrl(irFetchURL, "<fmt:message key="policy.url"/>");
            }
    ...
    function fillResourceImportDetailsForPolicies() {
        var filePath = document.getElementById('irFetchURL').value;
        var filename = resolvePolicyName(filePath);
    ...
    function fillResourceUploadDetails() {
        ...
        if (filename.search(/\.xml$/i) >= 0) {
            // so it is a single policy.
            document.getElementById('uMediaType').value = "application/policy+xml";
    ...
    function resolvePolicyName(filepath) {
        var filename = "";
    ...
    <carbon:breadcrumb
            label="policy.menu.text"
            resourceBundle="org.wso2.carbon.governance.policy.ui.i18n.Resources"
            topPage="true"
            request="<%=request%>"/>
    <script type="text/javascript">
    </script>
    <div id="middle">
        ...
        <h2><fmt:message key="add.policy"/></h2>
        <div id="workArea">

                <table class="styledLeft">
                    <thead>
                    <tr>
                        <th><fmt:message key="add.policy.table.heading"/></th>
                    </tr>
                    </thead>
                        ...
                        <td class="formRow">
                            <table width="100%">
                                <tr>
                                    <td>
                                        <div>
                                        <select id="addMethodSelector" onchange="viewAddPolicyUI()">
                                            <option value="import" <%= !isUploadError? "selected=\"selected\"" : "" %> ><fmt:message key="import.policy.from.url"/></option>
                                            <option value="upload" <%= isUploadError? "selected=\"selected\"" : "" %> ><fmt:message key="upload.policy.from.file"/></option>
                                        </select>
                                        </div>
                                        <br/>
                                        <div id ="importUI" <%= isUploadError? "style=\"display:none;\"" : "" %> >
                                            <form action="../resources/import_resource_ajaxprocessor.jsp" method="post"
                                                ...
                                                <input type="hidden" name="mediaType" value="application/policy+xml"/>
                                                <%--<input type="hidden" name="isAsync" value="true"/>--%>
                                                <table class="normal">
                                                    <tr>
                                                        <td><fmt:message key="policy.url"/> <span class="required">*</span></td>
                                                        <td><input type="text"
                                                                   onchange="fillResourceImportDetailsForPolicies()"
                                        ...
                                        <div id="uploadUI" <%= !isUploadError? "style=\"display:none;\"" : "" %> >
                                            <form method="post"
                                                 ...
                                                 <table class="normal">
                                                    <tr>
                                                        <td><fmt:message key="policy.file"/> <span class="required">*</span></td>
                                                        <td> <p>
                    ...
                    <tr id="buttonRow">
                        <td class="buttonRow">
                            <input class="button registryWriteOperation" type="button" onClick="addPolicy();"
                    ...
                    <tr id="waitMessage" style="display:none">
                        <td>
                            <div style="font-size:13px !important;margin-top:10px;margin-bottom:10px;margin-left:5px !important" class="ajax-loading-message"><img
                                    src="images/ajax-loader.gif" align="left" hspace="20"/><fmt:message key="please.wait.until.policy.is.added"/>...
                            </div>
                        </td>
                    </tr>
                </table>
