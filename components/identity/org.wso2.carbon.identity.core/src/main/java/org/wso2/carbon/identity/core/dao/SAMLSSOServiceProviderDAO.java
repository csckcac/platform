...
public class SAMLSSOServiceProviderDAO extends AbstractDAO<SAMLSSOServiceProviderDO> {
	...
	protected SAMLSSOServiceProviderDO resourceToObject(Resource resource) {
		...
		serviceProviderDO.setAssertionConsumerUrl(resource.getProperty(IdentityRegistryResources.PROP_SAML_SSO_ASSERTION_CONS_URL));
		...
		if (resource.getProperty(IdentityRegistryResources.PROP_SAML_SSO_USE_FULLY_QUALIFIED_USERNAME_AS_SUBJECT) != null) {
			serviceProviderDO.setUseFullyQualifiedUsername(new Boolean(
			                                                           resource.getProperty(IdentityRegistryResources.PROP_SAML_SSO_USE_FULLY_QUALIFIED_USERNAME_AS_SUBJECT)
			                                                                   .trim()));
		}

		if (resource.getProperty(IdentityRegistryResources.PROP_SAML_SSO_DO_SINGLE_LOGOUT) != null) {
			serviceProviderDO.setDoSingleLogout(new Boolean(
			                                                resource.getProperty(IdentityRegistryResources.PROP_SAML_SSO_DO_SINGLE_LOGOUT)
			                                                        .trim()));
		}
		...
		if (resource.getProperty(IdentityRegistryResources.PROP_SAML_SSO_DO_SIGN_ASSERTIONS) != null) {
			serviceProviderDO.setDoSignAssertions(new Boolean(
			                                                  resource.getProperty(IdentityRegistryResources.PROP_SAML_SSO_DO_SIGN_ASSERTIONS)
			                                                          .trim()));
		}
	...
	public boolean addServiceProvider(SAMLSSOServiceProviderDO serviceProviderDO)
		...
		if (serviceProviderDO.getIssuer() != null) {
			path =
			       IdentityRegistryResources.SAML_SSO_SERVICE_PROVIDERS +
		...
		try {
			...
			String useFullyQualifiedUsername =
			                                   serviceProviderDO.isUseFullyQualifiedUsername()
			                                                                                  ? "true"
			                                                                                  : "false";
			resource.addProperty(IdentityRegistryResources.PROP_SAML_SSO_USE_FULLY_QUALIFIED_USERNAME_AS_SUBJECT,
	...
	public SAMLSSOServiceProviderDO[] getServiceProviders() throws IdentityException {
		...
		try {
			if (registry.resourceExists(IdentityRegistryResources.SAML_SSO_SERVICE_PROVIDERS)) {
				String[] providers =
				                     (String[]) registry.get(IdentityRegistryResources.SAML_SSO_SERVICE_PROVIDERS)
				                                        .getContent();
				if (providers != null) {
