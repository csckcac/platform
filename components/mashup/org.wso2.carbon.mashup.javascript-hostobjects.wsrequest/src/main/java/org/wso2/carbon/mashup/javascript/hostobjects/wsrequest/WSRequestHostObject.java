...
public class WSRequestHostObject extends ScriptableObject {
    ...
    public static Scriptable jsConstructor(Context cx, Object[] args, Function ctorObj,
        ...
        if (object instanceof AxisService) {
            ...
            Object resourceFileObject = axisService.getParameterValue(MashupConstants.RESOURCES_FOLDER);
            if (resourceFileObject != null && resourceFileObject instanceof File) {
                wsRequest.resourcesDir = (File) resourceFileObject;
            } else {
    ...
    public static void jsFunction_open(Context cx, Scriptable thisObj, Object[] arguments,
        ...
        if (wsRequest.readyState > 0 && wsRequest.readyState < 4) {
            throw new CarbonException("Invalid readyState for WSRequest Hostobject : " + wsRequest.readyState);
        ...
        try {
            ConfigurationContext configurationContext =
                    WSRequestServiceComponent.getConfigurationContext();
        ...
        if (arguments.length == 1) {
            ...
            if (arguments[0] instanceof org.wso2.javascript.xmlimpl.QName) {
                ...
                if (wsRequest.targetNamespace == null) {
                    throw new CarbonException("The targetNamespace of the service is null, please specify a " +
                ...
                throw new CarbonException("Invalid parameter type for the WSRequest.send() method");
            }
            payload = arguments[1];
        } else {
            throw new CarbonException("Invalid no. of parameters for the WSRequest.send() method");
        }

        OMElement payloadElement = null;
        if (wsRequest.readyState != 1) {
            throw new CarbonException("Invalid readyState for the WSRequest Hostobject : " + wsRequest.readyState);
        }
        if (payload instanceof String) {
            ...
            try {
                ...
                if (node instanceof OMElement) {
                    ...
                    throw new CarbonException("Invalid input for the payload in WSRequest Hostobject : " + payload);
                }
            } catch (Exception e) {
                String message = "Invalid input for the payload in WSRequest Hostobject : " + payload;
                log.error(message, e);
    ...
    */
        ...
        if(optionsArray != null) {
            ...
            if (mepObject != null && !(mepObject instanceof Undefined) &&
                ...
                if (IN_OUT.equalsIgnoreCase(mepValue) || IN_ONLY.equalsIgnoreCase(mepValue)) {
                    ...
                    throw new CarbonException("Invalid value for mep. Supported values are in-out and in-only");
                }
            }
            ...
            for (int i = 0; i < wsRequest.soapHeaders.getLength(); i++) {
                ...
                if (soapHeaderObject instanceof String) {
                    ...
                    if (soapHeader.get("qName",
                        ...
                        throw new CarbonException("No qName property found for the soap headers");
                    }
    ...
    private static NativeArray setOptionsOpen(WSRequestHostObject wsRequest, Object[] args) throws CarbonException {
    ...
    private static NativeArray setOptionsOpenWSDL(WSRequestHostObject wsRequest, Object[] args) throws CarbonException {
        NativeArray optionsArray = null;
        String wsdlURL;
    ...
    private static CryptoConfig getCryptoConfig(WSRequestHostObject wsRequest, NativeObject crypto) throws CarbonException {
        Properties merlinProp = new Properties();
        File file = new File(wsRequest.resourcesDir.getAbsolutePath(),
                FilenameUtils.normalizeNoEndSeparator(getObjectProperty(crypto, "file")));
        merlinProp.put("org.apache.ws.security.crypto.merlin.file",
                file.getAbsolutePath().substring(CarbonUtils.getCarbonHome().length() + 1));
        ...
        if ((property instanceof Boolean && (Boolean) property) ||
            ...
            throw new CarbonException("Invalid value for property '" + CryptoConfig.CACHE_ENABLED +
        ...
        if (property instanceof Integer) {
            ...
            cryptoConfig.setCacheRefreshInterval((String) property);
                ...
                !(property instanceof UniqueTag)) {
            throw new CarbonException("Invalid value for property '" + CryptoConfig.CACHE_REFRESH_INTVL +
                    "' in rampart configuration");
        }
        return cryptoConfig;
    ...
    private static String filterProperty(WSRequestHostObject wsRequest, String property) {
        String resourcePath = wsRequest.resourcesDir.getAbsolutePath().substring(
                CarbonUtils.getCarbonHome().length() + 1);
    ...
    private static void setRampartConfigs(WSRequestHostObject wsRequest, QName operationName)
        ...
        if (policy != null) {
            if (rampartConfig != null) {
                ...
                throw new CarbonException("A policy has been specified either in your mashup or in the WSDL. " +
    ...
    private static void filterCryptoConfig(WSRequestHostObject wsRequest, CryptoConfig config) {
        ...
        properties.setProperty("org.apache.ws.security.crypto.merlin.file",
                filterProperty(wsRequest, properties.getProperty("org.apache.ws.security.crypto.merlin.file")));
    }
}
