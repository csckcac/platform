    ...
    #footer {text-align:center; font-size:75%}
        ...
        <body onload='init()'>
            ...
            <div id="footer">
                ...
                <div>Access all the operations of the <xsl:value-of select="$service"/> service through the <a href="../{substring-before($src, concat($service, '?'))}services/{$service}?tryit">Try-it page</a>.</div>
            </div>
        </body>
    </html>
</xsl:template>
