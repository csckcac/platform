...
public class RegistryManager {
    ...
    /**
     * when getting hosts from registry getHostsFromRegistry method will return all the
     * hosts in the registry. As like addHostToRegistry will add the hostinfo to the registry
     * @param hostName
    ...
    public void updateHostToRegistry(Host host, String webApp) throws Exception {
        try {
            ...
            if (registryService.resourceExists(hostResourcePath)) {
                ...
                hostResource.setProperty(
                        UrlMapperConstants.HostProperties.HOST_NAME, host.getName());
                hostResource.setProperty(
                        UrlMapperConstants.HostProperties.WEB_APP, webApp);
     ...
     * @throws Exception
     */
    public Resource getHostFromRegistry(String hostName) throws Exception {
        String hostResourcePath = UrlMapperConstants.HostProperties.HOSTINFO + hostName;
        if (registryService.resourceExists(hostResourcePath)) {
    ...
    public String getWebAppNameForHost(String hostName) throws Exception {
        Resource resource = getHostFromRegistry(hostName);
        if (resource != null) {
    ...
    public Collection getHostsFromRegistry() throws Exception {
