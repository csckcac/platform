    ...
    public void populate(ClassLoaderResolver clr, ClassLoader primary, MetaDataManager mmgr)
    {
        ...
        if (md instanceof AbstractMemberMetaData)
        {
            ...
            if (embCmd == null)
            {
                ...
                throw new InvalidMetaDataException(LOCALISER, "044121", apmd.getFullFieldName(), apmd.getTypeName());
            }
        }
        else if (md instanceof ElementMetaData)
        {
            ...
            if (embCmd == null)
            {
                NucleusLogger.METADATA.error(LOCALISER.msg("044122", apmd.getFullFieldName(), 
                    apmd.getCollection().getElementType()));
                throw new InvalidMetaDataException(LOCALISER, "044122", apmd.getFullFieldName(), apmd.getTypeName());
            }
        }
        else if (md instanceof KeyMetaData)
        {
            ...
            if (embCmd == null)
            {
                NucleusLogger.METADATA.error(LOCALISER.msg("044123", apmd.getFullFieldName(),
                    apmd.getMap().getKeyType()));
                throw new InvalidMetaDataException(LOCALISER, "044123", apmd.getFullFieldName(), embeddedType);
            }
        }
        else if (md instanceof ValueMetaData)
        {
            ...
            if (embCmd == null)
            {
                NucleusLogger.METADATA.error(LOCALISER.msg("044124", apmd.getFullFieldName(), 
                    apmd.getMap().getValueType()));
                throw new InvalidMetaDataException(LOCALISER, "044124", apmd.getFullFieldName(), embeddedType);
            }
        }
            ...
            for (int i=0;i<cls_fields.length;i++)
            {
                    ...
                    !Modifier.isStatic(cls_fields[i].getModifiers()))
                {
                    ...
                    if (Collections.binarySearch(members, cls_fields[i].getName()) < 0)
                    {
                        ...
                        if (omittedFmd != null)
                        {
                            NucleusLogger.METADATA.debug(LOCALISER.msg("044125", cls_fields[i].getName(), 
                                embeddedType, apmd.getClassName(true)));
        ...
        while (memberIter.hasNext())
        {
            ...
            else
            {
                ...
                catch(Exception e)
                {
                    ...
                    {
                        // MetaData field doesn't exist in the class!
                        throw new InvalidMetaDataException(LOCALISER, "044071", embeddedClass.getName(),
                            fieldFmd.getFullFieldName());
                    }
                }
