...
public class OSGiPluginRegistry implements PluginRegistry {
    ...
    public void registerExtensions()
    {
        ...
        for (org.osgi.framework.Bundle osgiBundle : osgiBundles)
        {
            ...
            if (pluginURL == null ) {
		pluginURL = osgiBundle.getResource("plugin.xml");
            }
        ...
        for (int i = 0; i < registeringExtensions.size(); i++)
        {
            ...
            if (exPoint == null)
            {
                if (extension.getPlugin() != null && extension.getPlugin().getSymbolicName() != null && extension.getPlugin()
                        .getSymbolicName().startsWith(DATANUCLEUS_PKG))
                {
                    NucleusLogger.PLUGIN.warn(LOCALISER.msg("024002", extension.getExtensionPointId(), extension.getPlugin()
                            .getSymbolicName(), extension.getPlugin().getManifestLocation().toString()));
                }
            }
            else
            {
    ...
    protected void registerExtensionPointsForPluginInternal(List extPoints, boolean updateExtensionPointsArray)
    {
        ...
        while (pluginExtPointIter.hasNext())
        {
            ...
            extensionPointsByUniqueId.put(exPoint.getId(), exPoint);
        }
    ...
    private Bundle registerBundle(org.osgi.framework.Bundle osgiBundle)
    {
        ...
        if (registeredPluginByPluginId.get(bundle.getSymbolicName()) == null)
        {
            ...
            {
                NucleusLogger.PLUGIN
                        .debug("Registering bundle " + bundle.getSymbolicName() + " version " + bundle.getVersion() + " at URL " + bundle
                                .getManifestLocation() + ".");
            }
