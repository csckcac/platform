...
public class ExpressionCompiler
{
    ...
    public Expression compileOrderExpression(Node node)
    {
        if (isOperator(node, "order"))
        {
            ...
            {
                return new OrderExpression(compileExpression(node.getFirstChild()), (String) node.getNextChild().getNodeValue());
            }
            if (node.getChildNodes().size() == 1)
            {
                return new OrderExpression(compileExpression(node.getFirstChild()));
            }
        }
