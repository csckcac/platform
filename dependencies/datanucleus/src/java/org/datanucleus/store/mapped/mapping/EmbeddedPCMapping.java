...
public class EmbeddedPCMapping extends EmbeddedMapping implements MappingCallbacks
{
    ...
    private ObjectProvider getStateManagerForEmbeddedObject(ObjectProvider ownerSM)
    {
        ...
        if (mmd.getParent() instanceof EmbeddedMetaData)
        {
            ...
            AbstractClassMetaData cmd = ownerSM.getExecutionContext().getMetaDataManager().getMetaDataForClass(mmd.getClassName(), clr);
        ...
        ObjectProvider thisSM = ownerSM.getExecutionContext().findObjectProvider(value);
        if (thisSM == null)
        {
            // Assign a StateManager to manage our embedded object
            thisSM = ObjectProviderFactory.newForEmbedded(ownerSM.getExecutionContext(), value, false, ownerSM, theMmd.getAbsoluteFieldNumber());
