...
public class HiveConf extends Configuration {
        ...
        if (hiveSiteURL == null && carbonHome != null) {
            try {
                hiveSiteURL = new URL("file://" + carbonHome + File.separator + "repository" +
                                      ...
                                      File.separator +
                                      "hive-site.xml");
                System.out.println("Path : " + "file://" + carbonHome + File.separator + "repository" +
                                   File.separator + "conf" + File.separator + "advanced" +
                                   File.separator +
                                   "hive-site.xml");
    ...
    private void processConnectionURLs() {
        String metaStoreURL = this.get(ConfVars.METASTORECONNECTURLKEY.varname);

        String carbonHome = System.getProperty("CARBON_HOME");

        if (metaStoreURL != null && metaStoreURL.contains("${CARBON_HOME}")) {
            metaStoreURL.replace("${CARBON_HOME}", carbonHome);
        }

        this.set(ConfVars.METASTORECONNECTURLKEY.varname, metaStoreURL);

        String statStoreURL = this.get(ConfVars.HIVESTATSDBCONNECTIONSTRING.varname);

        if (statStoreURL != null && statStoreURL.contains("${CARBON_HOME}")) {
            statStoreURL.replace("${CARBON_HOME}", carbonHome);
        }

        this.set(ConfVars.HIVESTATSDBCONNECTIONSTRING.varname, statStoreURL);
