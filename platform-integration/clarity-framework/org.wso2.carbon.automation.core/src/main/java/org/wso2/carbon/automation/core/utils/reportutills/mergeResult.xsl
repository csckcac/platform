<xslt:transform xmlns:xslt="http://www.w3.org/1999/XSL/Transform"
                xmlns:m="http://informatik.hu-berlin.de/merge" version="1.0"
                exclude-result-prefixes="m" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                xsi:schemaLocation="http://informatik.hu-berlin.de/merge ">
            ...
            <xslt:otherwise>
                ...
                <xslt:choose>
                    ...
                    $first1 is *not* an empty text node  -->
                            <xslt:when
                                    test="contains($diff-rest,'=') and not($type1='text' and normalize-space($first1)='')">
...
text node "=" otherwise a text node "!" -->
    <xslt:template name="m:compare-nodes">
        ...
        <xslt:choose>
            ...
            <xslt:when
                    test="$type1='element' and $type2='element' and local-name($node1)=local-name($node2) and namespace-uri($node1)=namespace-uri($node2) and name($node1)!=$dontmerge and name($node2)!=$dontmerge">
                ...
                <xslt:variable name="diff-att">
                    ...
                    <xslt:for-each select="$node1/@*">
                        <xslt:if
                                test="not($node2/@* [local-name()=local-name(current()) and namespace-uri()=namespace-uri(current()) and .=current()])">
                            .
                        </xslt:if>
                    </xslt:for-each>
                </xslt:variable>
            ...
            <xslt:when
                    test="$type1!='element' and $type1=$type2 and name($node1)=name($node2) and ($node1=$node2 or ($normalize='yes' and normalize-space($node1)= normalize-space($node2)))">
                =
            </xslt:when>
