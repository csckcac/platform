...
public class DataProvider {
    ...
    public DataProvider() throws ClassNotFoundException, SQLException {
        if (con == null || con.isClosed()) {
            String jdbcUrl = this.getProperty("jdbc.url");
    ...
    public JSONArray getTestCases(String build, String state) throws SQLException, IOException {
        ...
        ResultSet rst = con.executeQuery("SELECT WA_TESTCASE_STAT.*, WA_TEST_CLASS_STAT.WA_TEST_CLASS_NAME " +
    ...
    public JSONArray getTestCases(int testClass) throws SQLException, IOException {
        JSONArray objArray = new JSONArray();
        ResultSet rst = con.executeQuery("SELECT WA_TESTCASE_STAT.*, WA_TEST_CLASS_STAT.WA_TEST_CLASS_NAME FROM " +
    ...
    public JSONObject getTestSuite(int suiteId) throws SQLException {
        ...
        ResultSet rst = con.executeQuery("SELECT * FROM WA_TEST_SUITE_DETAIL WHERE WA_TEST_SUITE_ID = " + suiteId);
        while (rst.next()) {
    ...
    public JSONArray getTestSuites(int build) throws SQLException {
        ...
        ResultSet rst = con.executeQuery("SELECT * FROM WA_TEST_SUITE_DETAIL WHERE WA_BUILD_NUMBER = " + build);
        while (rst.next()) {
    ...
    public JSONArray getTestClasses(int build, int suite) throws SQLException {
        JSONArray buildInfo = new JSONArray();
        ResultSet rst = con.executeQuery("SELECT * FROM WA_TEST_CLASS_STAT WHERE WA_BUILD_NUMBER = "
    ...
    public JSONObject getTestClass(int classId) throws SQLException {
        JSONObject object = null;
        ResultSet rst = con.executeQuery("SELECT * FROM WA_TEST_CLASS_STAT WHERE WA_TEST_CLASS_ID = " + classId);
        while (rst.next()) {
            object = new JSONObject();
            object.put("build", rst.getString("WA_BUILD_NUMBER"));
            object.put("suite", rst.getString("WA_TEST_SUITE_ID"));
    ...
    public JSONArray getBuildHistory() throws SQLException {
        ...
        ResultSet rst = con.executeQuery("SELECT * FROM WA_BUILD_HISTORY ORDER BY WA_BUILD_NUMBER DESC");
        while (rst.next()) {
    ...
    public JSONObject getBuild(String buildNo) throws SQLException {
        ...
        ResultSet rst = con.executeQuery("SELECT * FROM WA_BUILD_HISTORY WHERE WA_BUILD_NUMBER=" + buildNo);
        while (rst.next()) {
    ...
    public JSONObject getLastBuild() throws SQLException {
        JSONObject record = null;
        ResultSet rst = con.executeQuery("SELECT * FROM WA_BUILD_HISTORY " +
    ...
    private String getProperty(String key) {
        ...
        InputStream is = DataProvider.class.getClassLoader().getResourceAsStream("dbconf.properties");
        try {
    ...
    private static int getCount(int build, String status) throws SQLException {
        ResultSet rst = con.executeQuery("SELECT count(WA_TESTCASE_STATUS) FROM WA_TESTCASE_STAT " +
    ...
    private static double getTimeTaken(int build)
            throws SQLException {
        ResultSet rst = con.executeQuery("SELECT sum(WA_TEST_DURATION) FROM WA_TEST_SUITE_DETAIL " +
                                         "WHERE WA_BUILD_NUMBER = " + build);
        double record = 0;
        while (rst.next()) {
    ...
    public static Hashtable<String, String> getFailReason(int testCaseId)
            throws SQLException, IOException {
        ResultSet rst = con.executeQuery("SELECT * FROM WA_ERROR_DETAIL " +
    ...
    public static String getStackTrace(int exceptionId) throws SQLException, IOException {
        ResultSet rst = con.executeQuery("SELECT WA_ERROR_DESCRIPTION FROM WA_ERROR_DETAIL " +
    ...
    public static JSONObject getTimeHistory(int suite, String testClassName) throws SQLException {
        ResultSet rst = con.executeQuery("SELECT WA_BUILD_NUMBER,WA_TEST_DURATION FROM WA_TEST_CLASS_STAT " +
