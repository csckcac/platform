...
public class GSStratosSeleniumTestSuite extends MasterTestSuite {
    @AfterSuite
    public void suiteRunner() {
        ...
        if (env.getFrameworkSettings().getEnvironmentSettings().is_runningOnStratos()) {
            suiteVariablesList.add(new SuiteVariables("GSPortalViewTest", GSPortalViewTest.class));
        }

        superSuite("StratosGadgetServerTestSuite", suiteVariablesList).run();
    }
    ...
    public static void main(String[] args) {
        EnvironmentBuilder env = new EnvironmentBuilder();
        if (env.getFrameworkSettings().getEnvironmentSettings().is_runningOnStratos()) {
            new GSStratosSeleniumTestSuite().suiteRunner();
