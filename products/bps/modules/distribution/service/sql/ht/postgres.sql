CREATE TABLE DEADLINE (id BIGINT NOT NULL, DEADLINE_DATE ABSTIME NOT NULL, NAME VARCHAR(255) NOT NULL, STATUS_TOBE_ACHIEVED VARCHAR(255) NOT NULL, TASK_ID BIGINT, PRIMARY KEY (id));
CREATE TABLE EVENT (id BIGINT NOT NULL, DETAILS VARCHAR(255), NEW_STATE VARCHAR(255), OLD_STATE VARCHAR(255), TIMESTAMP ABSTIME NOT NULL, TYPE VARCHAR(255) NOT NULL, USER VARCHAR(255) NOT NULL, TASK_ID BIGINT, PRIMARY KEY (id));
CREATE TABLE GENERIC_HUMAN_ROLE (GHR_ID BIGINT NOT NULL, type VARCHAR(23), TASK_ID BIGINT, PRIMARY KEY (GHR_ID));
CREATE TABLE HTJOB (id BIGINT NOT NULL, DETAILS VARCHAR(4096), NAME VARCHAR(255), NODEID VARCHAR(255), SCHEDULED BOOL NOT NULL, TASKID BIGINT NOT NULL, TIME BIGINT NOT NULL, TRANSACTED BOOL NOT NULL, TYPE VARCHAR(255) NOT NULL, PRIMARY KEY (id));
...
CREATE TABLE TASK (id BIGINT NOT NULL, ACTIVATION_TIME ABSTIME, COMPLETE_BY_TIME ABSTIME, CREATED_ON ABSTIME, ESCALATED BOOL, EXPIRATION_TIME ABSTIME, NAME VARCHAR(255) NOT NULL, PRIORITY INTEGER NOT NULL, SKIPABLE BOOL, START_BY_TIME ABSTIME, STATUS VARCHAR(255) NOT NULL, STATUS_BEFORE_SUSPENSION VARCHAR(255), TENANT_ID INTEGER NOT NULL, TYPE VARCHAR(255) NOT NULL, UPDATED_ON ABSTIME, FAILURE_MESSAGE BIGINT, INPUT_MESSAGE BIGINT, OUTPUT_MESSAGE BIGINT, PARENTTASK_ID BIGINT, PRIMARY KEY (id));
CREATE TABLE TASK_ATTACHMENT (id BIGINT NOT NULL, ACCESS_TYPE VARCHAR(255), ATTACHED_AT ABSTIME, CONTENT_TYPE VARCHAR(255), ATTACHMENT_NAME VARCHAR(255), VALUE VARCHAR(255), TASK_ID BIGINT, ATTACHED_BY BIGINT, PRIMARY KEY (id));
CREATE TABLE TASK_COMMENT (id BIGINT NOT NULL, COMMENT_TEXT VARCHAR(8192), COMMENTED_BY VARCHAR(100), COMMENTED_ON ABSTIME, MODIFIED_BY VARCHAR(100), MODIFIED_ON ABSTIME, TASK_ID BIGINT, PRIMARY KEY (id));
