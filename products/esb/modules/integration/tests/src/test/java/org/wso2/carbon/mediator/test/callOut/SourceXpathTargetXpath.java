...
public class SourceXpathTargetXpath {
    ...
    public void setEnvironment() throws Exception, IOException {
        ...
        caseUtils.loadESBConfigurationFromClasspath
                ("/artifacts/ESB/mediatorconfig/callout/synapse_sample_430.xml", backEndUrl, sessionCookie);
    }
    ...
    @Test (alwaysRun = true)
        ...
        response = axis2Client.sendSimpleStockQuoteRequest("http://localhost:8280/","","IBM");
         // send the simplestockquote request. service url is set at the synapse
        boolean ResponseContainsIBM = response.getFirstElement().toString().contains("IBM");
         //checks whether the  response contains IBM
