/*
 * Copyright 2004,2005 The Apache Software Foundation.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.wso2.carbon.registry.ws.client.test.general;

import org.apache.axis2.context.ConfigurationContextFactory;
import org.apache.commons.io.FileUtils;
import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import org.wso2.carbon.base.ServerConfiguration;
import org.wso2.carbon.integration.core.FrameworkSettings;
import org.wso2.carbon.integration.core.ServerUtils;
import org.wso2.carbon.integration.core.TestTemplate;
import org.wso2.carbon.registry.core.Registry;
import org.wso2.carbon.registry.ws.client.registry.WSRegistryServiceClient;

import java.io.File;

public class TestSetup extends TestTemplate {

    static Registry registry = null;
    static boolean isInitialized = false;

    private static final Log log = LogFactory.getLog(TestSetup.class);
    public static final String POLICY_FILE_PATH = File.separator + "repository" + File.separator +
            "conf" + File.separator + "ws-api-sec-policy.xml";

    @Override
    public void setUp() throws Exception {
        if (!isInitialized && Boolean.parseBoolean(System.getProperty("run.with.security"))) {
            String carbonZip = System.getProperty("carbon.zip");
            if (carbonZip == null) {
                throw new IllegalArgumentException("carbon zip file is null");
            }
            String carbonHome = ServerUtils.setUpCarbonHome(carbonZip);
            if (carbonHome != null) {
                FrameworkSettings.getProperty();
                frameworkPath = FrameworkSettings.getFrameworkPath();
                FileUtils.copyFile(new File(
                        frameworkPath + File.separator + ".." +
                                File.separator + "src" + File.separator + "test" + File.separator +
                                "java" + File.separator +
                                "resources" + File.separator + "policy.xml"), new File(
                        carbonHome + TestSetup.POLICY_FILE_PATH));
                File[] libs = new File(frameworkPath + File.separator + ".." + File.separator +
                        "target" + File.separator + "lib").listFiles();
                for (File lib : libs) {
                    if (lib.getName().startsWith("rampart")) {
                        FileUtils.copyFileToDirectory(lib,
                                new File(carbonHome + File.separator + "repository" +
                                        File.separator + "deployment" +
                                        File.separator + "client" + File.separator + "modules"));
                        break;
                    }
                }
                ServerUtils.instrumentJarsForEmma(carbonHome);
                ServerUtils.startServerUsingCarbonHome(carbonHome);
            }
            isInitialized = true;
        }
        super.setUp();
    }

    @Override
    public void init() {
        log.info("Initializing WS-API Tests");
        try {
            registry = new WSRegistryServiceClient("https://" + FrameworkSettings.HOST_NAME + ":" +
                    FrameworkSettings.HTTPS_PORT + "/services/", FrameworkSettings.USER_NAME,
                    FrameworkSettings.PASSWORD,
                    ConfigurationContextFactory.createConfigurationContextFromFileSystem(
                            FrameworkSettings.CARBON_HOME +
                                    File.separator + "repository" + File.separator + "deployment" +
                                    File.separator + "client",
                            ServerConfiguration.getInstance().getFirstProperty("Axis2Config.clientAxis2XmlLocation")));
            if (Boolean.parseBoolean(System.getProperty("run.with.security"))) {
                ((WSRegistryServiceClient)registry).addSecurityOptions(
                        FrameworkSettings.CARBON_HOME + TestSetup.POLICY_FILE_PATH,
                        FrameworkSettings.CARBON_HOME + File.separator +
                                "repository" + File.separator + "resources"
                                + File.separator + "security" + File.separator + "wso2carbon.jks");
            }

        } catch (Exception e) {
            e.printStackTrace();
            fail("Failed to authenticate the client. Caused by: " + e.getMessage());
        }
        log.info("WS-API Tests Initialized");
    }

    @Override
    public void cleanup() {
    }

    @Override
    public void runSuccessCase() {
    }

    @Override
    public void runFailureCase() {
    }
}
