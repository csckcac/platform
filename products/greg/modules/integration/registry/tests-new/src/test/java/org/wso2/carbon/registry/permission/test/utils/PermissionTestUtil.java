...
public class PermissionTestUtil {
    ...
    private static ManageEnvironment nonAdminEnvironment;
    private static UserManagementClient userManagementClient;

    private static void setupManageEnvironments() throws LoginAuthenticationExceptionException, RemoteException {
        if (adminEnvironment == null) {
            EnvironmentBuilder builderAdmin = new EnvironmentBuilder().greg(0);
            adminEnvironment = builderAdmin.build();
        }

        if (nonAdminEnvironment == null) {
