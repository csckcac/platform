...
public class CopyResourceCommunityFeaturesTestCase {
    ...
    public void testCopyResource() throws ResourceAdminServiceExceptionException, RemoteException {
        ...
        String authorUserName = resourceAdminClient.getResource(RES_COPIED_LOCATION + RES_NAME_AFTER_COPYING)[0].getAuthorUserName();
        assertTrue(userInfo.getUserName().equalsIgnoreCase(authorUserName), "Collection copying failure");
    }

    @Test(dependsOnMethods = "testCopyResource",description = "Checks the dependencies after copying the collection")
    public void testDependenciesAfterCopying() throws AddAssociationRegistryExceptionException, RemoteException {
        String dependencyType = "depends";
        ...
        assertTrue((RES_COPIED_LOCATION + RES_NAME_AFTER_COPYING).equalsIgnoreCase(bean.getAssociationBeans()[0].getSourcePath()),
                "Source dependency is not correct after copying.");

    }

    @Test(dependsOnMethods = "testDependenciesAfterCopying",
    ...
    @Test(dependsOnMethods = "testAssociationsAfterCopying",
    ...
    @Test(dependsOnMethods = "testCommentsAfterCopying",
    ...
    @Test(dependsOnMethods = "testLifecycleAfterCopying",description = "Checks the rating after copying the collection")
    public void testRatingAfterCopying() throws RegistryException, RegistryExceptionException {
        int userRating = infoServiceAdminClient.getRatings(RES_COPIED_LOCATION + RES_NAME_AFTER_COPYING, sessionId).getUserRating();
    ...
    @Test(dependsOnMethods = "testRatingAfterCopying",
    ...
    public void testSubscriptionsAfterCopying() throws RegistryException, RegistryExceptionException {
        SubscriptionBean sBean = infoServiceAdminClient.getSubscriptions(RES_COPIED_LOCATION + RES_NAME_AFTER_COPYING, sessionId);

        assertNotNull(sBean.getSubscriptionInstances(), "Error retrieving subscriptions  after copying.");
    }

    @Test(dependsOnMethods = "testLifecycleAfterCopying",description = "Checks the tags after copying the collection")
    public void testTagsAfterCopying() throws RegistryException, RegistryExceptionException {
        String tag =
                infoServiceAdminClient.getTags(RES_COPIED_LOCATION + RES_NAME_AFTER_COPYING, sessionId).getTags()[0].getTagName();
