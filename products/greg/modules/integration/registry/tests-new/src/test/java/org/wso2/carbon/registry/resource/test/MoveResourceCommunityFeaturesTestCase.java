...
public class MoveResourceCommunityFeaturesTestCase {
    ...
    @Test(dependsOnMethods = "testAddDependencyToResource")
    ...
    @Test(dependsOnMethods = "testAddTag", enabled = true)
    ...
    @Test(dependsOnMethods = "testAddCommentToResource")
    ...
    public void testMoveResource() throws ResourceAdminServiceExceptionException, RemoteException {
        ...
        String authorUserName = resourceAdminClient.getResource(RES_MOVED_LOCATION + RES_NAME_AFTER_MOVING)[0].getAuthorUserName();
        assertTrue(userInfo.getUserName().equalsIgnoreCase(authorUserName), "Collection copying failure");
    }

    @Test(dependsOnMethods = "testMoveResource",description = "Checks the dependencies after copying the collection")
    public void testDependenciesAfterMoving() throws AddAssociationRegistryExceptionException, RemoteException {
        ...
        assertTrue((RES_MOVED_LOCATION + RES_NAME_AFTER_MOVING).equalsIgnoreCase(bean.getAssociationBeans()[0].getSourcePath()),
                "Source dependency is not correct after copying.");

    }

    @Test(dependsOnMethods = "testDependenciesAfterMoving",
    ...
    public void testAssociationsAfterMoving() throws AddAssociationRegistryExceptionException, RemoteException {
        ...
        AssociationTreeBean aTreeBean = relationServiceClient.getAssociationTree(RES_MOVED_LOCATION + RES_NAME_AFTER_MOVING);

        assertTrue(aTreeBean.getAssociationTree().contains(ASSOCIATION_PATH),
                "Association is not correct after copying.");
    }

    @Test(dependsOnMethods = "testAssociationsAfterMoving",
    ...
    @Test(dependsOnMethods = "testCommentsAfterMoving",
            description = "Checks the Lifecycle after copying the collection")
    public void testLifecycleAfterMoving()
            throws LifeCycleManagementServiceExceptionException, RemoteException, RegistryException {
    ...
    @Test(dependsOnMethods = "testLifecycleAfterMoving",description = "Checks the rating after copying the collection")
    public void testRatingAfterMoving() throws RegistryException, RegistryExceptionException {

        int userRating = infoServiceAdminClient.getRatings(RES_MOVED_LOCATION + RES_NAME_AFTER_MOVING, sessionId).getUserRating();
    ...
    @Test(dependsOnMethods = "testRatingAfterMoving",
            description = "Checks the subscriptions after copying the collection", enabled=false)
    public void testSubscriptionsAfterCopying() throws RegistryException, RegistryExceptionException {

        SubscriptionBean sBean = infoServiceAdminClient.getSubscriptions(RES_MOVED_LOCATION + RES_NAME_AFTER_MOVING, sessionId);

        assertNotNull(sBean.getSubscriptionInstances(), "Error retrieving subscriptions  after copying.");
    }

    @Test(dependsOnMethods = "testLifecycleAfterMoving",description = "Checks the tags after copying the collection")
    public void testTagsAfterCopying() throws RegistryException, RegistryExceptionException {

        String tag =
                infoServiceAdminClient.getTags(RES_MOVED_LOCATION + RES_NAME_AFTER_MOVING, sessionId).getTags()[0].getTagName();
