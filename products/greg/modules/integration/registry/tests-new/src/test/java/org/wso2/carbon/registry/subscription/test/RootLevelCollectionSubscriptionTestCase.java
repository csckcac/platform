...
public class RootLevelCollectionSubscriptionTestCase {
	...
	private String collectionPath = "/_system";
	...
	public void testSubscribeCollectionMgtConsole() throws RegistryException, RemoteException {
		InfoServiceAdminClient infoServiceAdminCllient = new InfoServiceAdminClient(environment.getGreg().getProductVariables().getBackendUrl(), userInfo.getUserName(),userInfo.getPassword());
		SubscriptionBean bean = infoServiceAdminCllient.subscribe(collectionPath, "work://SubscriptionTestRole", "CollectionUpdated", loggedInSessionCookie);
	...
	public void testJMXCollectionSubscription() throws RegistryException, RemoteException {
		InfoServiceAdminClient infoServiceAdminCllient = new InfoServiceAdminClient(environment.getGreg().getProductVariables().getBackendUrl(), userInfo.getUserName(),userInfo.getPassword());
		SubscriptionBean bean = infoServiceAdminCllient.subscribe(collectionPath, "jmx://", "CollectionUpdated", loggedInSessionCookie);
		assertTrue(bean.getSubscriptionInstances() != null);
	}
	
		
	@Test(groups = "wso2.greg" , description = "Update Collection" , dependsOnMethods = {"testSubscribeCollectionMgtConsole" , "testJMXCollectionSubscription"})
	public void testUpdateCollection() throws Exception {
		jmxClient = new JMXClient();
		jmxClient.connect(userInfo.getUserName(),userInfo.getPassword());jmxClient.registerNotificationListener(collectionPath);
	...
	public void testGetNotification() throws RemoteException, IllegalStateFault, IllegalAccessFault, IllegalArgumentFault, InterruptedException  {
		...
		for (WorkItem workItem : workItems) {
			if((workItem.getPresentationSubject().toString()).equals("The collection at path "+collectionPath+" was updated."))
    		 {
				success = true;
    			break;
    		 }
