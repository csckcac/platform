...
public class RootLevelResourceSubscriptionTestCase {
    ...
    public void testMgtConsoleResourceSubscription(String resourceName)
        ...
        SubscriptionBean bean = infoServiceAdminCllient.subscribe(resourcePathName + resourceName, "work://RoleSubscriptionTest", "ResourceUpdated", loggedInSessionCookie);
    ...
    public void testJMXResourceSubscription(String resourceName)
            throws RegistryException, RemoteException {
        InfoServiceAdminClient infoServiceAdminCllient = new InfoServiceAdminClient(environment.getGreg().getProductVariables().getBackendUrl(), userInfo.getUserName(), userInfo.getPassword());
        SubscriptionBean bean = infoServiceAdminCllient.subscribe(resourcePathName + resourceName, "jmx://", "ResourceUpdated", loggedInSessionCookie);
        assertTrue(bean.getSubscriptionInstances() != null);
    }
    ...
    @Test(groups = "wso2.greg", description = "Update resource", dependsOnMethods = {"testMgtConsoleResourceSubscription", "testJMXResourceSubscription"})
