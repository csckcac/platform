...
public class UploadIncompatibleCarFileTestCase {
    ...
    public void uploadIncompatibleCApplication()
        ...
        String[] appList = adminServiceApplicationAdmin.listAllApplications(sessionCookie);
        if (appList != null) {
    ...
    /*@Test(description = "Delete Carbon Application ", dependsOnMethods = {"uploadIncompatibleCApplication"})
    public void deleteCApplication()
            throws ApplicationAdminExceptionException, RemoteException, InterruptedException {
        adminServiceApplicationAdmin.deleteApplication(sessionCookie, "erro_content-1.0.0");
        Thread.sleep(20000);
    }*/

    @Test(priority = 2, description = "Upload CApp having Text Resources")
    public void uploadIncompatibleServerRoleCApplication()
        ...
        if (appList != null) {
            boolean isFound = false;
            for (String cApp : appList) {
                if ("incorrectServerRole".equalsIgnoreCase(cApp)) {
                    isFound = true;
                    break;
                }
            }
    ...
    /*@Test(description = "Delete Carbon Application ", dependsOnMethods = {"uploadIncompatibleServerRoleCApplication"})
    public void deleteIncompatibleServerRoleCApplication()
            throws ApplicationAdminExceptionException, RemoteException, InterruptedException {
        adminServiceApplicationAdmin.deleteApplication(sessionCookie, "serverRole-incorrect_1.0.0");
        Thread.sleep(20000);
    }*/
