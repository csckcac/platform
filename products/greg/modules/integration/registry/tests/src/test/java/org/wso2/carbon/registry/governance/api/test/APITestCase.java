...
public class APITestCase {
    ...
    public void initTest() {
        ...
        try {
            ...
            configPath = FrameworkSettings.getFrameworkPath() + File.separator + ".." + File.separator + ".." + File.separator + ".." +
                    ...
                    "resources" + File.separator + "api.metadata.xml";
    ...
    public void testAddAPI() throws Exception {
        ...
        GenericArtifact artifact = manager.newGovernanceArtifact(new QName("MyAPI"));
    ...
    @Test(groups = {"wso2.greg"})
    public void testAPIContentXMLInvalid() throws RegistryException,
            XMLStreamException {
        GovernanceUtils.loadGovernanceArtifacts((UserRegistry)registry);
        GenericArtifactManager manager = new GenericArtifactManager(registry, "api");
    ...
    public void testAPISearch() throws Exception {
        ...
        GenericArtifact artifact2 = manager.newGovernanceArtifact(new QName("NewAPI"));
        ...
        GenericArtifact artifact3 = manager.newGovernanceArtifact(new QName("NewAPI2"));
        ...
        GenericArtifact artifact4 = manager.newGovernanceArtifact(new QName("NewAPI3"));
        ...
        artifact2.setQName(new QName("NewAPIX"));
    ...
    @Test(groups = {"wso2.greg"})
    public void testAPIRename() throws Exception {
        GovernanceUtils.loadGovernanceArtifacts((UserRegistry)registry);
        GenericArtifactManager manager = new GenericArtifactManager(registry, "api");

        GenericArtifact artifact = manager.newGovernanceArtifact(new QName("MyAPIName"));
        artifact.addAttribute("testAttribute", "somevalue");
        artifact.addAttribute("overview_provider", "WSO2");
        artifact.addAttribute("overview_version", "1.0.0");
        manager.addGenericArtifact(artifact);

        artifact.setQName(new QName("RenameAPI"));
        manager.updateGenericArtifact(artifact);

        GenericArtifact exactCopy = manager.getGenericArtifact(artifact.getId());
        QName qname = exactCopy.getQName();

        Assert.assertEquals(new QName("RenameAPI"), qname);
        Assert.assertEquals(exactCopy.getPath(), "/apimgt/applicationdata/provider/WSO2/RenameAPI/1.0.0/api");
        ...
        newArtifact.setQName(new QName("NewRenameAPI"));
        manager.updateGenericArtifact(newArtifact);

        exactCopy = manager.getGenericArtifact(newArtifact.getId());
        qname = exactCopy.getQName();

        Assert.assertEquals(qname, new QName("NewRenameAPI"));
        Assert.assertEquals(exactCopy.getPath(), "/apimgt/applicationdata/provider/WSO2/NewRenameAPI/1.0.0/api");
    ...
    public void testAPIDelete() throws Exception {
        ...
        artifact = manager.newGovernanceArtifact(new QName("MyAPIName"));
        artifact.addAttribute("overview_provider", "WSO2");
        artifact.addAttribute("overview_version", "1.0.0");
        manager.addGenericArtifact(artifact);
